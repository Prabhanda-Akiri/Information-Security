...
def display_results(plain_text_blocks,wrapped_blocks,signed_blocks,recovered_message,signatures,calculated_signatures,plain_text):
	...
	for i in range(len(plain_text_blocks)):
		print('\nBlock:	',i+1)
		print('Original Message(M):		',plain_text_blocks[i])
		print('Message wrapped by attacker(M_dash=M*r^e mod N):		',wrapped_blocks[i])
		print('Signed message from Bob(S=M^d *r mod N):			',signed_blocks[i])
		print('Signature of Bob(S_dash=M^d mod N):				',signatures[i])
		print('calculated_signatures for verification(M^d mod N):		',calculated_signatures[i])

	if signatures==calculated_signatures:
		print('\n\nBob\'s signature  and calculated Bob\'s Signature are matching..!\nBlinding attack is Successful\n')
