...
def main():
	...
	plain_text_binary=block_formation()
	...
	store_result(ciphered_text)

def store_result(text):
	file=open('Output_Program_3.txt',"w")
...
def apply_DES(plain_text_binary,round_keys):
	...
	#initial_permuted_blocks=initial_permutation(plain_text_binary)
...
def block_formation():
	...
	file_no=int(input('Choose the plain text:\n1. plain text-1\n2. plain text-2\n3. plain text-3\n4. plain text-4\n5. plain text-5\n'))

	files_list=['PT-TC1.txt','PT-TC2.txt','PT-TC3.txt','PT-TC4.txt','PT-TC5.txt']
	...
	for i in range(0,length,64):
		...
		while j<64:
			if i+j<length:
				#plain_text[i+j]=int(plain_text[i+j])
				strg.append(int(plain_text[i+j]))
				j+=1
			else:
				strg.append(" ")
				j+=1
...
def get_round_keys():
	...
	files_list=['RK-TC1.txt','RK-TC2.txt','RK-TC3.txt','RK-TC4.txt','RK-TC5.txt']
